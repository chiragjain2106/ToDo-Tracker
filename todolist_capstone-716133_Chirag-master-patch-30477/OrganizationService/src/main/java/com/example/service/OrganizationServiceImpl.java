...
public class OrganizationServiceImpl implements OrganizationService {
    ...
    @Override
    public List<Task> serachTaskByCategory(String email, String category) {
        User user = organizationRepo.findByEmail(email);
        List<Task> taskList = user.getTaskList();
...
//            System.out.println("Task date : "+task.getDate());
            System.out.println("Date check : "+date1.equals(task.getDate())+",   Task date : "+task.getDate());
    ...
    @Override
    public List<Task> serachTaskByDate(String email, LocalDate date) {
        User user = organizationRepo.findByEmail(email);
        List<Task> taskList = user.getTaskList();
        List<Task> taskListByDate = new ArrayList<>();
        for (Task task:taskList) {
